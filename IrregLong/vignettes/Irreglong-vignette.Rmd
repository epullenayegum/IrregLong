...
i <- iiw.weights(Surv(time.lag,time,event)~Wt + Apgar + 
      ...
      invariant=c("Subject","Wt","Apgar"),lagvars=c("time","conc"),maxfu=16*24,lagfirst=c(0,0),first=FALSE)
i$m
 
```

Since Apgar score and birthweight are not significant, we consider whether the model can be simplified - first by including Apgar score as an indicator ($\geq 5$ vs. $<5$), then by removing variables that are not significant. We then consider whether interaction terms are needed.
```{r}
