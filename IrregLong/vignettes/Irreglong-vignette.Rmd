...
i <- iiw.weights(Surv(time.lag,time,event)~Wt + Apgar + I(conc.lag>0) + conc.lag + dose.lag +
      ...
      invariant="Subject",lagvars=c("time","conc"),maxfu=16*24,lagfirst=0,first=FALSE)
i$m
```
...
i <- iiw.weights(Surv(time.lag,time,event)~Wt *( I(conc.lag>0) + conc.lag) + dose.lag +
      ...
      invariant="Subject",lagvars=c("time","conc"),maxfu=16*24,lagfirst=0,first=FALSE)
i$m
i <- iiw.weights(Surv(time.lag,time,event)~Wt *dose.lag + I(conc.lag>0) + conc.lag +
      cluster(Subject),id="Subject",time="time",event="event",data=data,
      invariant="Subject",lagvars=c("time","conc"),maxfu=16*24,lagfirst=0,first=FALSE)
i$m
i <- iiw.weights(Surv(time.lag,time,event)~Wt *(dose.lag + I(conc.lag>0)) + conc.lag +
...
i$m
```
It thus appears that last dose, birthweight, last concentration, and the interactions between last dose and birthweight and last dose and concentration are predictive of the time to the next concentration measurement.
...
```{r}
iiwgee <- iiwgee(conc ~ I(time^3) + log(time),Surv(time.lag,time,event)~Wt *dose.lag + 
...
data$wt[order(data$id)] <- i$iiw.weight
mw <- geeglm(conc ~ I(time^3) + log(time) , id=Subject, data=data, weights=wt)
summary(mw)
```
...
m <- geeglm(conc ~ I(time^3) + log(time) , id=Subject, data=data)
time <- (1:200)
...
plot(data$time,data$conc,xlim=c(0,200),pch=16,xlab="Time",ylab="Serum phenobarbital concentration")
...
reg <- function(data){
  ...
  if(max(table(data$id))>1) est[,2] <- GEE.var.md(conc~I(time^3) + log(time), id=id,data=data)$cov.beta
  est <- data.matrix(est)
  return(est)
}
set.seed(301031)
...
data$ApgarInd.time3 <- as.numeric(data$ApgarInd)*(data$time^3)
data$ApgarInd.logtime <- as.numeric(data$ApgarInd)*log(data$time)
...
miiwgee <- geeglm(conc ~ ApgarInd *(time3 + log(time)),id=id,weight=wt,data=data)
